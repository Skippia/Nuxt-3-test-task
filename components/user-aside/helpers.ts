...
export function buildFilterAndSortedUsersData(
  props: { filterString: string; sortingMode: UserSearchSortedModeEnum },
  usersData: Ref<Omit<TUserFull, "comment">[] | null>,
) {
  const filteredUsersData = computed(() =>
    usersData.value?.filter((userData) =>
      `${userData.first_name}${userData.last_name}`
        .toLowerCase()
        .trim()
        .includes(props.filterString),
    ),
  );
