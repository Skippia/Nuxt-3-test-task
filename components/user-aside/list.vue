...
const { filteredAndSortedUsersData } = buildFilterAndSortedUsersData(
  props,
  usersData,
);
...
watch(
  usersData,
  (newUsersData) => {
    if (newUsersData) loadAndMergeAllPointsFromLC();
  },
  {
    immediate: true,
  },
...
watch(updatedUserCardEvents.value, () => {
  ...
  if (lastUpdatedUserCardEvent) {
    const targetUserData = usersData.value?.find(
      (userData) => userData.id === lastUpdatedUserCardEvent.id,
    );
    if (targetUserData) targetUserData.points = lastUpdatedUserCardEvent.points;
  }
});
</script>
...
<template>
  <ul class="w-full flex flex-col bg-white">
    <template v-if="!usersErrror">
      <ClientOnly>
        <UserAsideCard
          v-for="userData in filteredAndSortedUsersData"
          :key="userData.id"
          v-bind="userData"
        />
      </ClientOnly>
