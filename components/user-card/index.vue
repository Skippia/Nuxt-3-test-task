...
<template>
  <section
    class="flex flex-col items-center gap-y-3 rounded-lg bg-white p-6 shadow-md hover:bg-gray-50 hover:shadow-lg"
  >
    <div class="flex flex-col items-center justify-center gap-x-6 lg:flex-row">
      <img
        class="my-2 h-25 w-25 rounded-xl sm:(h-35 w-35)"
        :src="props.avatar"
        alt="User Image"
      />
      ...
      <div class="flex flex-col gap-y-1">
        ...
        <textarea
          v-model="proxyComment"
          class="my-3 h-60px w-full resize-none border-2 border-gray/500 rounded p-2 text-base sm:h-80px"
        />
      </div>
    </div>
    ...
    <button
      ...
      @click="
        emit('save-user-card', { comment: proxyComment, points: proxyPoints })
      "
    >
      Save
    </button>
