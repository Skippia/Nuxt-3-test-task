...
export const useGlobalState = createGlobalState(() => {
  ...
  const pushUpdatedUserCardEvent = (newEvent: TUpdatedUserCardEvent) => {
    const idx = updatedUserCardEvents.value.findIndex(
      (e) => e.id === newEvent.id,
    );

    if (idx !== -1) updatedUserCardEvents.value[idx] = newEvent;
    else updatedUserCardEvents.value.push(newEvent);
  };

  return {
    updatedUserCardEvents,
    popUpdatedUserCardEvent,
