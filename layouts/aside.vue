...
<template>
  <main class="h-full min-h-100vh flex bg-gray-300">
    <ClientOnly>
      <NuxtLink
        v-if="isClientMobilePage"
        class="absolute left-0 top-0 flex items-center justify-start gap-x-4 rounded bg-white p-3 text-base text-black font-medium duration-300"
        to="/"
      >
      ...
      <AsideMenu v-else class="h-full w-full">
        <template #tabs>
          <UserAsideTabs
            :is-active-clients="isSearchVisible"
            @update-search-visibility="updateSearchVisibility"
          />
        </template>
