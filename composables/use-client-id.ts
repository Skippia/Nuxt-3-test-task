...
export function useClientId(currentUserData: Ref<TUserFull | null>) {
  ...
  function loadCurrentCommentAndPointsFromLC() {
    // 1. Load data from LC
    usersDataStateLC.value = useLocalStorage(
      "usersDataMiscellaneous",
      [] as TUserMiscellaneous[],
    ).value;
    const targetUserDataLC = usersDataStateLC.value?.find(
      (userMiscellaneous) => userMiscellaneous.id === currentUserData.value?.id,
    );
  ...
  function updatePointsForCurrentUserData(newPointsVal: number) {
    ...
    const idx = usersDataStateLC.value?.findIndex(
      (userMiscellaneous) => userMiscellaneous.id === currentUserData.value?.id,
    );
  ...
  function mergeUserData() {
    ...
    const userMiscellaneous = usersDataStateLC.value?.find(
      (userMiscellaneous) => userMiscellaneous.id === currentUserData.value?.id,
    );
  ...
  return {
    ...
    updatePointsForCurrentUserData,
    updateCommentForCurrentUserData,
    mergeUserData,
  };
}
